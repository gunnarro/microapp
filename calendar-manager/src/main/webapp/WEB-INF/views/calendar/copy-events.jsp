				...
				<form:form modelAttribute="inputData" method="POST" id="calendar-event-copy-form">
					...
					<div class="form-group">
						...
						<form:select cssClass="form-control" path="fromDate">
							<c:forEach var="item" items="${inputData.items}">
								<fmt:formatDate var="formattedStartDate" value="${item.startDate}" pattern="EEEE, dd MMM yyyy" />
								<form:option value="${item.startDate}" label="${formattedStartDate}"/>
							</c:forEach>
						</form:select>
					</div>
