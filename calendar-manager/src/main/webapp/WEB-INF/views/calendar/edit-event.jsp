			...
			<div class="panel-heading clearfix">
				<c:if test="${not empty calendarevent.id}">
					<div class="pull-right">
						<div class="pull-right">
							<spring:url value="/calendar/event/copy/{eventId}" var="copyUrl">
								<spring:param name="eventId" value="${calendarevent.id}" />
							</spring:url>
							<a href="${fn:escapeXml(copyUrl)}" class="btn btn-primary btn-sm">Copy</a>
			...
			<div class="panel-body">
				<form:form modelAttribute="calendarevent" method="${method}" id="calendar-event-form">
					...
					<c:choose>
						<c:when test="${calendarevent['new']}">
							<div class="form-group">
								...
								<form:input disabled="true" cssClass="form-control" path="name" value="${calendarevent.name}" />
							</div>
						</c:when>
						<c:otherwise>
							<div class="form-group">
					...
					<div class="form-group ${status.error ? 'has-error' : ''}">
						<label for="startDate" class="control-label"><spring:message code="label.date"/></label>
						<div class="controls">
							<div class="input-group">
								<input id="startDate" name="startDate" type="text" class="date-picker form-control" value="${formattedStartDate}" placeholder="dd.MM.yyyy" onchange="updateEndDate();"/>
								<label for="startDate" class="input-group-addon btn">
									<span class="glyphicon glyphicon-calendar"></span>
								</label>
							</div>
						</div>
						<form:errors path="startDate" cssClass="help-block error-color" />
					</div>
					...
					<div class="form-group">
						...
						<form:select cssClass="form-control" path="type" >
							<form:option value="meeting">Meeting</form:option>
					...
					<c:if test="${calendarevent['new']}">
						<spring:bind path="endDate">
							<div class="form-group ${status.error ? 'has-error' : ''}">
								...
								<form:input id="endDate" cssClass="form-control" path="endDate" value="${formattedEndDate}" placeholder="dd.MM.yyyy"/>
