...
public class DietManagerRepositoryImpl extends BaseJdbcRepository implements DietManagerRepository {
	...
	public List<String> getSelecedMealNamesForDate(Date forDate) {
		...
		return getJdbcTemplate().query(query.toString(), new Object[] { forDateStr },
	...
	public int checkUserDietMenuItemLnk(Integer userId, Date createdDate, String mealName) {
		try {
			...
			return getJdbcTemplate().queryForObject(sqlQuery.toString(), new Object[] { createdDateStr, mealName },
					Integer.class);
	...
	public int createUserDietMenuItemLnk(Integer userId, MenuItem menuItem) {
		if (menuItem.isNew()) {
			return createLink(UserDietMenuItemLnkTable.TABLE_NAME, UserDietMenuItemLnkTable.getFKColumnNames(),
					new Object[] { userId, menuItem.getId(), menuItem.getControlledByUserId(),
							menuItem.getPreparedByUserId(), menuItem.getCausedConflict(), menuItem.getFkLogId() });
		} else {
			try {
				if (LOG.isDebugEnabled()) {
				...
				KeyHolder keyHolder = new GeneratedKeyHolder();
				getJdbcTemplate().update(UserDietMenuItemLnkTable.createInsertPreparedStatement(userId, menuItem),
						keyHolder);
				return keyHolder.getKey().intValue();
			} catch (Exception e) {
				LOG.error(null, e);
	...
	public int deleteUserDietMenuItemLnk(Integer userId, Integer dietMenuItemId) {
		return deleteLink(UserDietMenuItemLnkTable.TABLE_NAME, UserDietMenuItemLnkTable.getFKColumnNames(),
				new Object[] { userId, dietMenuItemId });
	}

	@Override
	public List<Rule> getAllDietRules() {
		return getJdbcTemplate().query("SELECT * FROM diet_rules r ORDER BY r.rule_name DESC", new Object[] {},
	...
	public List<ChartData> getBmiReferenceData() {
		...
		query.append(
				"SELECT b.id, CAST(b.month AS CHAR) AS label, b.bmi_p25 AS value1, b.bmi_p50 AS value2, b.bmi_p75 AS value3 ");
	...
	public ReferenceData getGrowthReferenceDataForDateOfBirth(Integer userId) {
		try {
			...
			return getJdbcTemplate().queryForObject(query.toString(), new Object[] {},
					DietManagerRowMapper.mapToReferenceDataRM());
	...
	public BodyMeasurementStatistic getBodyMeasurementStatistic(Integer userId, Integer forDays) {
		try {
			...
			return getJdbcTemplate().queryForObject(query.toString(), new Object[] { userId, forDays },
					DietManagerRowMapper.mapToBodyMeasurementStatisticRM());
		} catch (org.springframework.dao.EmptyResultDataAccessException erae) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Error: " + erae.toString());
			}
	...
	public HealthLogEntry getBodyMeasurementLog(Integer userId, Date logDate) {
		try {
			return getJdbcTemplate().queryForObject(
					"SELECT * FROM body_measurements_log l WHERE l.fk_user_id = ? AND l.log_date = ?",
	...
	public DietMenu getDietMenu(int menuId) {
		...
		try {
			...
			dietMenu = getJdbcTemplate().queryForObject(dieltPlanQuery.toString(), new Object[] { menuId },
					DietManagerRowMapper.mapToDietMenuRM());
		} catch (org.springframework.dao.EmptyResultDataAccessException erae) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Error: " + erae.toString());
			}
		...
		Map<String, List<MenuItem>> map = getJdbcTemplate().query(menuItemsQuery.toString(), new Object[] { menuId },
				new MenuItemsExtractor());
	...
	public MenuItem getDietMenuItem(Integer menuItemId) {
		try {
			return getJdbcTemplate().queryForObject("SELECT * FROM diet_menu_items WHERE id = ?",
					new Object[] { menuItemId }, DietManagerRowMapper.mapToDietMenuItemRM());
		} catch (org.springframework.dao.EmptyResultDataAccessException erae) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Error: " + erae.toString());
			}
	...
	public List<Type> getDietMenuItemTypes() {
		return getJdbcTemplate().query("SELECT DISTINCT menu_item_name AS name, 0 AS id FROM diet_menu_items",
				new Object[] {}, DietManagerRowMapper.mapToTypeRM());
	}
	...
	public DietPlan getDietPlan(Integer dietPlanId) {
		...
		try {
			...
			if (tmpDietPlanId == -1) {
				tmpDietPlanId = 1;
				dieltPlanQuery.append(
						"SELECT DISTINCT * FROM diet_plans WHERE diet_plan_active = ? ORDER BY created_date_time ASC");
			} else {
				dieltPlanQuery.append("SELECT * FROM diet_plans WHERE id = ?");
			}
			dietPlan = getJdbcTemplate().queryForObject(dieltPlanQuery.toString(), new Object[] { tmpDietPlanId },
					DietManagerRowMapper.mapToDietPlanRM());
		} catch (org.springframework.dao.EmptyResultDataAccessException erae) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Error: " + erae.toString());
			}
	...
	public List<DietPlan> getDietPlans(Integer userId) {
		return getJdbcTemplate().query("SELECT * FROM diet_plans", new Object[] {},
				DietManagerRowMapper.mapToDietPlanRM());
	}
	...
	public List<FoodProduct> getDietProductChangeList() {
		StringBuilder query = new StringBuilder();
		query.append(
				"SELECT p.id, p.product_name, p.product_type, p.product_description,i.id, i.product_equivalent_description");
		...
		for (FoodProduct p : products) {
			List<KeyValuePair> productEquivalents = getJdbcTemplate().query(
					"SELECT * from diet_product_equivalent_items WHERE fk_diet_product_id = ?",
					new Object[] { p.getId() },
	...
	public Rule getDietRule(Integer id) {
		try {
			return getJdbcTemplate().queryForObject("SELECT * FROM diet_rules r WHERE r.id = ?", new Object[] { id },
					DietManagerRowMapper.mapToRuleRM());
		} catch (org.springframework.dao.EmptyResultDataAccessException erae) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Error: " + erae.toString());
	...
	public List<Rule> getDietRules(Integer dietPlanId) {
		return getJdbcTemplate().query(
				"SELECT * FROM diet_rules r WHERE r.fk_diet_plan_id = ? ORDER BY r.rule_name DESC",
				new Object[] { dietPlanId }, DietManagerRowMapper.mapToRuleRM());
	}
	...
	public List<MealStatistic> getMealStatistic(Integer userId, Integer days) {
		return getJdbcTemplate().query(DietManagerSql.createMealStatisticQuery(),
				new Object[] { userId, userId, userId, userId }, DietManagerRowMapper.mapToMealStatisticRM(userId));
	}
	...
	public List<KeyValuePair> getMealsManagedByUserStatistic(Integer days) {
		return getJdbcTemplate().query(DietManagerSql.createMealsManagedByUserQuery(), new Object[] {},
				DietManagerRowMapper.mapToKeyValueRM(FIELD_WEEKNUMBER, FIELD_USERNAME,
						FIELD_MEALS_CONTROLLED_BY_USER_COUNT));
	}
	...
	public List<KeyValuePair> getMealsPreparedByUserStatistic(Integer days) {
		return getJdbcTemplate().query(DietManagerSql.createMealsPreparedByUserQuery(), new Object[] {},
				DietManagerRowMapper.mapToKeyValueRM(FIELD_WEEKNUMBER, FIELD_USERNAME,
						FIELD_MEALS_PREPARED_BY_USER_COUNT));
	}
	...
	public List<String> getMenuItemSelectionTrend(Integer userId, Integer menuItemId, Integer days) {
		...
		query.append(" LIMIT ?");
		return getJdbcTemplate().query(query.toString(), new Object[] { menuItemId, days },
				DietManagerRowMapper.singelValueRM("created_date_time"));
	}
	...
	public List<DietMenu> getMenus(Integer userId) {
		return getJdbcTemplate().query("SELECT * FROM diet_menus", new Object[] {},
				DietManagerRowMapper.mapToDietMenuRM());
	}
	...
	public Map<Date, List<String>> getMissingMealsForUser(Integer id, Integer forLastDays) {
		...
		List<String> defaultMeals = getJdbcTemplate().query(
				"SELECT meal_name FROM diet_plan_meals WHERE meal_default = 1", new Object[] {},
				DietManagerRowMapper.mapToStringValueRM("meal_name"));
		List<KeyValuePair> list = getJdbcTemplate().query(DietManagerSql.createDroppedMealsOverviewQuery(),
				new Object[] { forLastDays }, DietManagerRowMapper.mapToKeyValueRM("created_date", "meals"));
		if (LOG.isDebugEnabled()) {
			LOG.debug(list.toString());
		}
		// Loop through all days with missing meals
		for (KeyValuePair k : list) {
			List<MenuItem> thisDayMeals = getJdbcTemplate().query(DietManagerSql.createGetMealsForDate(),
					new Object[] { k.getKey() }, DietManagerRowMapper.mapToMenuItemRM());
			if (LOG.isDebugEnabled()) {
	...
	public int getNumberOfActivitiesLastDays(Integer days) {
		return getJdbcTemplate().queryForObject(DietManagerSql.createLogLevelCountQuery(),
				new Object[] { days, "ACTIVITY" }, DietManagerRowMapper.mapCountRM());
	}

	@Override
	public int getNumberOfConflictsLastDays(Integer days) {
		return getJdbcTemplate().queryForObject(DietManagerSql.createLogLevelCountQuery(),
				new Object[] { days, "CONFLICT" }, DietManagerRowMapper.mapCountRM());
	...
	public int getNumberOfMealsLastDays(Integer days) {
		...
		return getJdbcTemplate().queryForObject(query.toString(), new Object[] { days },
	...
	public Product getProduct(Integer productId) {
		try {
			return getJdbcTemplate().queryForObject("SELECT * FROM products WHERE id = ?", new Object[] { productId },
					ProductTable.mapToProductRM());
		} catch (org.springframework.dao.EmptyResultDataAccessException erae) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Error: " + erae.toString());
			}
	...
	public List<Product> getProducts() {
		return getJdbcTemplate().query("SELECT * FROM products p ORDER BY p.name, p.category DESC", new Object[] {},
				ProductTable.mapToProductRM());
	}
	 ...
	 * item_count, (SELECT concat(i.menu_item_name,' ',i.menu_item_description) FROM
	 * diet_menu_items i WHERE i.id = l.fk_diet_menu_item_id) FROM
	 * user_diet_menu_item_lnk l GROUP BY l.fk_diet_menu_item_id ORDER BY item_count
	 * DESC LIMIT 10
	 * 
	 ...
	 * WHERE l.fk_controlled_by_user_id = u.id GROUP BY l.fk_controlled_by_user_id
	 * ORDER BY controlled_meal_count DESC
	 * 
	 * Missing meals: SELECT COUNT( date( l.created_date_time ) ) AS
	 * meal_count_pr_day, date( l.created_date_time ) FROM user_diet_menu_item_lnk l
	 * GROUP BY date( l.created_date_time ) HAVING meal_count_pr_day =5 ORDER BY
	 * l.created_date_time DESC LIMIT 0 , 30
	 */
	@Override
	public List<MenuItem> getSelectedMenuItemsForUser(Integer userId, Integer forDays) {
		...
		mealsQuery.append(
				",(SELECT m.id FROM diet_plan_meals m WHERE i.menu_item_name = m.meal_name) AS fk_menu_item_name_id");
		...
		return getJdbcTemplate().query(mealsQuery.toString(), new Object[] { forDays, forDays },
	...
	public List<KeyValuePair> getSummaryStatistic(Integer days) {
		return getJdbcTemplate().query(DietManagerSql.createSummaryStatisticQuery(), new Object[] {},
				DietManagerRowMapper.mapToKeyValueRM(FIELD_USERNAME, FIELD_USERNAME,
						FIELD_MEALS_CONTROLLED_BY_USER_COUNT));
	}
	...
	public Integer getUserSelectedMenuItemCount(Integer userId, Integer dietMenuItemId) {
		...
		query.append(" WHERE l.fk_user_id IN(1,2,3,4,5,6)");
		query.append(" AND l.fk_diet_menu_item_id = ?");
		return getJdbcTemplate().queryForObject(query.toString(), new Object[] { dietMenuItemId },
				DietManagerRowMapper.mapCountRM());
	}
	...
	public int updateDietMenuItem(MenuItem menuItem) {
		...
		return getJdbcTemplate().update(DietMenuItemTable.createUpdateQuery(),
				DietMenuItemTable.createUpdateParam(menuItem));
	}
	...
	public int createFollowerForUser(Integer userId, Integer userFollowerId) {
		return createLink(UserFollowerLnkTable.TABLE_NAME, UserFollowerLnkTable.getFKColumnNames(),
				new Object[] { userId, userFollowerId });
	}

	@Override
	public int deleteFollowerForUser(Integer userId, Integer userFollowerId) {
		return deleteLink(UserFollowerLnkTable.TABLE_NAME, UserFollowerLnkTable.getFKColumnNames(),
				new Object[] { userId, userFollowerId });
	}
	...
	private int createLink(String tableName, String[] columnNames, Object[] values) {
		...
		try {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Created new link in table: {} whith id: {}, for {}, {}", tableName,
						keyHolder.getKey().intValue(), values[0], values[1]);
			}
			...
			LOG.warn(null, e);
			LOG.warn("Duplicate entry! table: {}, {} = {}, {} = {}", tableName, columnNames[0], values[0],
					columnNames[1], values[1]);
			return 0;
		} catch (Exception sqle) {
			LOG.error("Error creating link! table: {}, {} = {}, {} = {}", tableName, columnNames[0], values[0],
					columnNames[1], values[1]);
			throw new ApplicationException("Error creating link! table=" + tableName + " " + columnNames[0] + " = "
					+ values[0] + ", " + columnNames[1] + " = " + values[1] + "\n" + sqle);
		}
	}
