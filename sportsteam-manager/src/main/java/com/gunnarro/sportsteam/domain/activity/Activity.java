...
public abstract class Activity extends BaseDomain implements Serializable {
    ...
    public boolean isFinished() {
        if (this.status != null) {
            return this.status.getName().equals(ActivityStatusEnum.COMPLETED.name());
        }
        // Status not set, check if start date is passed by 24 hours
        if (startDate != null) {
            // give 24 hour slack, 24*60*1000=1440000ms
            return (startDate.getTime() + 1440000) < System.currentTimeMillis();
        }
        return false;
    }
