...
import com.gunnarro.tournament.cache.CacheHandler;
...
public class TournamentPlannerServiceImpl implements TournamentPlannerService {
    ...
    public void saveTournament(Tournament tournament) {
        cacheHandler.putTournamentIntoCache(tournament.getTournamentName(), tournament);
    }
    ...
    public List<Tournament> getTournaments(String token) {
        return cacheHandler.getAllTournamentFromCache(token);
    }
    ...
    public Tournament getTournament(String tournamentId) {
        return cacheHandler.getTournamentFromCache(tournamentId);
    }
    ...
    public Tournament generateTournament(TournamentInput tournamentInput) {
        try {
            ...
            if (t != 0) {
                throw new RuntimeException("number og treams must be a even numer! teams=" + teamNames.size());
            }
            ...
            while (i <= numberOfTeams) {
                ...
                List<Match> generatedMatches = generateMatches(names);
            ...
            cacheHandler.putTournamentIntoCache(genTournament.getTournamentName(), genTournament);
     ...
     * @param playTime in minutes
     * @param pause in minutes
     */
    ...
    private void verifyNumberOfMatches(String name, List<Match> matches, int numberOfTeams) {
        ...
        LOG.debug("Number of matches per Team: " + map.entrySet().iterator().next().getValue());
        LOG.debug("Number of matches for " + name + " verified OK");
    }
